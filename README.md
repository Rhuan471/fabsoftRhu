// WELCOME TO YOUR NEW LIFE PROJECT
# Fábrica de Software 2025/1

# INICIO DA AULA
1) RODAR O SPRINGBOOT DASHBOARD
2) ABRIR O TERMINAL, DIGITAR

cd projfabsoft-frontend

3) DIGITAR

npm install (caso precise instalar para os pacotes)
ng serve



## Propostas de projeto

- Sistema de Gerenciamento de Voos
  - Criar e Acompanhar projetos
  - Gestão Financeira
  - Compra Inteligente
  - Backup na Nuvem
  - Migração de Sistemas
    

# Livro de Eng Software Moderna
[Eng. Soft Moderna - Requisitos](https://engsoftmoderna.info/cap3.html)

## Nome do projetor

  - Rhuan J Voltolini

### Instalação
[Git Bash](https://git-scm.com/downloads)

### Configuração inicial

```
git config --global user.name "NOME DO USUARIO NO GITHUB"
git config --global user.email "EMAIL DA CONTA DO GITHUB"
```
## Histórias de Usuários
- Como um Atendente, eu gostaria de registrar os dados do cliente como: nome, email, cpf, telefone
- Como um Atendente, eu gostaria de registrar os dados do avião na embarcação como: marca do modelo, data do voo, horário de decolagem e a quantidade de assentos disponíveis no avião
- Como um atendente, eu gostaria de vincular o avião ao passageiro

### Commit e Push

```
git add .
git commit -m "tipo: o que foi feito"
git push -u origin main
```

### Ambiente de desenvolvimento JAVA
[Eclipse Adoptium](https://adoptium.net/)
[Microsoft OpenJDK](https://www.microsoft.com/openjdk)
[AWS CORRETTO](https://aws.amazon.com/pt/corretto/)
[VSCode](https://code.visualstudio.com/download)
[Extension Pack for Java](https://marketplace.visualstudio.com/items?itemName=vscjava.vscode-java-pack)


- JRE - Java Runtime Environment
   - Java Virtual Machine JVM 
     - java.exe / javaw.exe
- JDK - Java Development Kit
  - Compilar o java (javac.exe)

